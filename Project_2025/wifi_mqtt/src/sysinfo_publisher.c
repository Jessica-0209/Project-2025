...
char *get_hostname()
{
	...
	{
		LOG_WARN("[SYSINFO] gethostname failed: %s", strerror(errno));
 ...
 *
 * Returns: CPU usage as a percentage (double), or -1 on error.
 */
...
double get_memory_usage()
{
	...
	LOG_DEBUG("[SYSINFO] Memory Usage: %.2f%% (used: %ld kB, total: %ld kB)", usage, used, total);
	return usage;
}
...
void *run_sysinfo_publisher(void *arg)
{
	...
	{
		LOG_ERROR("[SYSINFO] Failed to connect to broker at %s:%d", mqtt_config.mqtt_host, mqtt_config.mqtt_port);
